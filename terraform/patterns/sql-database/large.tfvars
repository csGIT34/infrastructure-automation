# Azure SQL Database - Large T-Shirt Size
# Use case: Production environments
# - Premium P2 SKU (250 DTUs)
# - 500GB storage
# - Zone redundant (HA across 3 availability zones)

project       = "myapp"
name          = "appdb"
business_unit = "engineering"
owners = ["sa_scottc1@azureskylab.com"]
location      = "eastus"

# Sizing
sku_name       = "P2"
max_size_gb    = 500
zone_redundant = true  # HA across 3 availability zones

# Features (always enabled)
enable_diagnostics   = true
enable_access_review = true

# Supporting infrastructure
log_analytics_workspace_id = "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-monitoring/providers/Microsoft.OperationalInsights/workspaces/law-main"

# NOTE: For multi-region HA, enable Active Geo-Replication or Failover Groups
# to replicate to a secondary region (e.g., westus) with automatic failover
