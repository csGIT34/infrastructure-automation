  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: {
          type: "string",
          description: "Action to perform: 'create' (default) or 'destroy'",
          enum: ["create", "destroy"],
          default: "create"
        }
      },
...
function generatePatternRequest(params: {
  ...
  const actionLabel = action === "destroy" ? "DESTROY" : "CREATE";
  const actionWarning = action === "destroy" ? `# ⚠️  WARNING: This will DESTROY resources!\n#\n` : "";
...
#
${actionWarning}${action === "create" ? `# Estimated monthly cost: $${costs}\n` : ""}# Size: ${size}
#
# What will be ${action === "destroy" ? "destroyed" : "provisioned"}:
...
function validateSingleDocument(config: any, index: number = 0): any {
  const errors: string[] = [];
  ...
  if (!config.metadata) {
    ...
    if (config.metadata.environment && !["dev", "staging", "prod"].includes(config.metadata.environment)) {
      errors.push(`Invalid environment '${config.metadata.environment}'`);
    }
  }
  ...
  if (!config.pattern) {
    ...
    errors.push(`Unknown pattern '${config.pattern}'`);
  } else {
...
function validatePatternRequest(yamlContent: string): string {
  ...
  try {
    ...
    const configs = documents.map(doc => doc.toJSON()).filter((c: any) => c !== null);

    if (configs.length === 0) {
      return JSON.stringify({
        valid: false,
        errors: ["No valid documents found in YAML"],
        warnings: [],
        provisioned_components: [],
        summary: "Invalid: empty YAML"
      }, null, 2);
    }

    // If multiple documents, validate each and return aggregate result
    if (configs.length > 1) {
      ...
      for (let i = 0; i < configs.length; i++) {
        const docResult = validateSingleDocument(configs[i], i);
        allResults.push(docResult);
        if (!docResult.valid) allValid = false;
        if (docResult.action === "create" && docResult.valid) createCount++;
        if (docResult.action === "destroy" && docResult.valid) destroyCount++;
      }

      return JSON.stringify({
        valid: allValid,
        document_count: configs.length,
        create_count: createCount,
        destroy_count: destroyCount,
        documents: allResults,
        summary: allValid
          ? `Valid: ${configs.length} document(s) - ${createCount} create, ${destroyCount} destroy`
          : `Invalid: ${allResults.filter(r => !r.valid).length} document(s) have errors`
      }, null, 2);
    }
      ...
      - name: Validate infrastructure YAML
        ...
        run: |
          ...
          for i, doc in enumerate(documents):
              errors, action = validate_document(doc, i)
      ...
      - name: Generate Plan Preview
        ...
        run: |
          ...
          if len(documents) > 1:
              ...
              preview += "| Order | Doc | Action | Pattern | Name |\\n"
              preview += "|-------|-----|--------|---------|------|\\n"
              order = 1
              # Destroy first
              for i, doc in destroy_docs:
                  name = doc.get('config', {}).get('name', 'N/A')
                  preview += f"| {order} | {i} | DESTROY | \`{doc.get('pattern', 'N/A')}\` | \`{name}\` |\\n"
                  order += 1
              # Then create
              for i, doc in create_docs:
                  name = doc.get('config', {}).get('name', 'N/A')
                  preview += f"| {order} | {i} | CREATE | \`{doc.get('pattern', 'N/A')}\` | \`{name}\` |\\n"
                  order += 1
              preview += "\\n---\\n\\n"

          # Process destroy docs first in preview
          for section_name, section_docs, icon in [
              ("Destroy Actions", destroy_docs, "DESTROY"),
              ("Create Actions", create_docs, "CREATE") if len(documents) > 1 else ("Pattern Request", create_docs, "")
          ]:
              if not section_docs:
                  continue
