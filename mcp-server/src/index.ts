...
const tools: Tool[] = [
  ...
  },
  {
    ...
    description: "Generate a GitHub Actions workflow file that processes infrastructure.yaml and triggers the provisioning pipeline. This workflow should be added to the user's repository.",
    inputSchema: {
      type: "object",
      properties: {
        api_url: {
          type: "string",
          description: "URL of the Infrastructure API Gateway",
          default: "https://your-api-gateway.azurewebsites.net"
        },
        environments: {
          type: "array",
          items: { type: "string" },
          description: "List of environments to support (e.g., ['dev', 'staging', 'prod'])",
...
function generateWorkflow(params: {
  ...
  const apiUrl = params.api_url || "https://your-api-gateway.azurewebsites.net";
  const environments = params.environments || ["dev", "staging", "prod"];
  const requireApproval = params.require_approval !== false;

  const workflow = `# Infrastructure Provisioning Workflow
# This workflow processes infrastructure.yaml and triggers the provisioning pipeline
# Generated by Infrastructure MCP Server
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate YAML syntax
        run: |
          python3 -c "import yaml; yaml.safe_load(open('infrastructure.yaml'))"
          echo "✅ YAML syntax is valid"

  submit:
    name: Submit Infrastructure Request
    needs: validate
    runs-on: ubuntu-latest
${requireApproval ? `    environment: \${{ needs.validate.outputs.environment }}` : ''}

    steps:
      - name: Checkout
        uses: actions/checkout@v4

      - name: Submit to Infrastructure API
        env:
          API_URL: ${apiUrl}
...
# ============================================================
#
# 1. Save this file as: .github/workflows/infrastructure.yaml
#
# 2. Add the following secret to your repository:
#    - INFRA_API_KEY: Your Infrastructure API authentication key
#
# 3. Create your infrastructure.yaml in the repository root:
#
#    metadata:
